...
def evaluate_RegGNN(sample_selection=False, shuffle=False, random_state=None,
    ...
    connectomes = inputs_gnn
    scores = targets_gnn
    connectomes = torch.tensor(connectomes.values)
    connectomes = torch.t(connectomes)
    scores = torch.tensor(scores.values)
    scores = torch.t(scores)

    pyg_data = []
    for subject in range(scores.shape[0]):
        sparse_mat = connectomes[subject, :]
        pyg_data.append(torch_geometric.data.Data(x=sparse_mat,
                                                  y=scores[subject, :].float(), edge_index=edge_index,
                                                  edge_attr=None))
